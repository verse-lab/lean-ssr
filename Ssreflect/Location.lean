...
partial def elabSsr (stx :  TSyntax `ssr_intro) : TacticM Unit := newTactic do
    ...
    | `(ssr_intro|/$t:term) => newTactic do
      ...
      run `(tactic| revert $name)
      run `(tactic| clear $name:ident)
    ...
    | `(ssr_intro| <-) => newTactic do
      ...
      run `(tactic| rw [<-$name:ident])
      tryGoal $ run `(tactic| clear $name:ident)
    | `(ssr_intro| ?) => newTactic do run `(tactic| intro _)
    | `(ssr_intro| *) => newTactic do run `(tactic| intros)
    | `(ssr_intro| >) => newTactic do introsDep
    | `(ssr_intro| _) => newTactic do
      let name ← fresh "H"
      run `(tactic| intro $name:ident)
      run `(tactic| clear $name:ident)
    | `(ssr_intro| {}) => newTactic do run `(tactic| scase)
    | `(ssr_intro| //) => newTactic do run `(tactic| try (intros; aesop) )
    | `(ssr_intro| /=) => newTactic do run `(tactic| try simp )
    | `(ssr_intro| //=) => newTactic do run `(tactic| try simp; try aesop )
    | `(ssr_intro| \) => throwError "Break Point"
    | `(ssr_intro| { $[$is:ssr_intros]|* } ) => do
      if (← getUnsolvedGoals).length == 1 then
        run `(tactic|scase)
      let goals ← getUnsolvedGoals
      if goals.length != is.size then
        throwError s!"Given { is.size } tactics, but excpected { goals.length }"
      else
